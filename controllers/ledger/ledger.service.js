...
let LedgerServices = class LedgerServices {
  ...
  getFilter() {
    return __awaiter(this, void 0, void 0, function*() {
      const id = userId;
      try {
        const partialFilter = yield this.sumInc.findID({ MemberID: id });
        return {
          status: 200,
  ...
  getSoa(params) {
    return __awaiter(this, void 0, void 0, function*() {
      ...
      try {
        const sumIncData = yield this.sumInc.getSummary(
          reqData.id,
          reqData.year,
          reqData.period
        );
        const PerSalesData = yield this.pSales.getPersonalSales(
          reqData.id,
          reqData.year,
          reqData.period
        );
        const goldData = yield this.goldM.getGoldmine(
          reqData.id,
          reqData.year,
          reqData.period
        );
        const diamondData = yield this.diaomondM.getDiamondmine(
          reqData.id,
          reqData.year,
          reqData.period
        );
        const gpdData = yield this.gpd.getGpd(
          reqData.id,
          reqData.year,
          reqData.period
        );
        const royaltyData = yield this.royaltyDisc.getRoyaltyDiscount(
          reqData.id,
          reqData.year,
          reqData.period
        );
...
__decorate(
  [mongo_1.Models("ledger"), __metadata("design:type", Object)],
  LedgerServices.prototype,
  "model",
  void 0
);
LedgerServices = __decorate(
  [
    ...
    __metadata("design:paramtypes", [
      cutoffDates_service_1.CutoffDatesServices,
